...
(function () {
  ...
  var MESSAGES = {
    ...
    // 예매만 한 사람 한 자리다. 경매에서는 좌석을 몇 개든 살 수 있으므로
    // 이 문구가 "한 사람에 한 자리" 로 끝나면 뒤 단계와 정면으로 모순된다.
    ALREADY_HAVE_SEAT:      '이미 예매한 좌석이 있습니다. 예매는 한 자리까지입니다.',
    ...
    // 돈이 '없다' 가 아니라 '걸려 있다' 고 말해야 한다. 입찰해도 잔액은
    // 줄지 않는다(정산에서 빠진다). 그래서 학생 눈에는 돈이 그대로
    // 보이는데 버튼만 죽어 있고, 이유를 말해 주지 않으면 영영 모른다.
    NO_MONEY:               '다른 좌석에 걸어 둔 돈까지 합치면 가진 금액을 넘습니다.',
    OVER_COMMITTED:         '다른 좌석에 이미 걸어 둔 돈이 있습니다. 그만큼은 지금 쓸 수 없습니다.',
    SEAT_LIMIT:             '한 사람이 살 수 있는 좌석 수를 이미 채웠습니다.',
    // 마감과 정산이 겹친 순간. 규칙 위반이 아니라 '다시 누르면 되는' 쪽이다.
    BUSY:                   '지금 정산 중입니다. 한 번 더 눌러 주세요.',
